// src/pages/api/comments/moderate.ts
...
export const POST: APIRoute = async ({ request }) => {
  try {
    const { commentId, commentIds, action, reason } = await request.json();
    ...
    const idsToProcess = commentIds || [commentId];
    ...
    const moderationLogs = idsToProcess.map(commentId => ({
