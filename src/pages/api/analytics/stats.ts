...
export const GET: APIRoute = async ({ url }) => {
  try {
    ...
    // Comment statistics
    const { data: comments, error: commentsError } = await db.comments()
      .select('*')
    ...
    // User statistics
    const { data: users, error: usersError } = await db.profiles()
    ...
    const { data: moderationActions, error: modError } = await db.commentModeration()
      .select('*')
      .gte('created_at', startDate.toISOString());
